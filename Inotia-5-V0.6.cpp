...
long long MP,maxMP,ATK,fang,LV=1,G=0,mu=50,gang=5,sheng=20,xilian=0,chou=500;
float baoji=2.5;
int stop_time=27;
...
int monsterHP[22]={0,0,650,1050,950,1250,0,0,1300,1650,1750,1950,2250,2850,3050,3450,4050,4950,5550,6550,7850,12950};
int monsterATK[22]={0,0,245,305,355,405,0,0,455,525,605,655,715,775,845,905,1085,1235,1465,1815,2435,4855};
...
void Read()
{
	...
	scanf("%lld%lld",&HP,&maxHP);
	scanf("%lld%lld",&MP,&maxMP);
	scanf("%lld",&ATK);
	scanf("%lld",&fang);
	scanf("%lld%lld",&EXP,&maxEXP);
	scanf("%d",&LV);
	scanf("%lld",&jinbi);
	scanf("%lld",&G);
	scanf("%d",&choosejn);
	...
	scanf("%d",&rate);
	scanf("%d",&TD);
	scanf("%d%d",&assTD[0],&assTD[1]);
	scanf("%d%d%d",&bag,&bag1,&bag2);
	for(int i=0;i<=9;i++)
	{
		scanf("%d",&Size);
		char cc=getchar();
	...
	{
		cc=getchar();
		zhuangbeifangju[i-1]=cc;
	}
	scanf("%d",&zhuangbeif);
	scanf("%d",&pct);
	scanf("%d%d%d",&mu,&sheng,&gang);
	scanf("%d",&xilian);
	scanf("%d",&chou);
	freopen("CON","r",stdin);
	Sleep(10000);
...
void POF()
{
	...
	for(int i=0;i<=9;i++)
	{
		cout<<daoju[i].size()<<" "<<daoju[i]<<endl;
	}
	for(int i=0;i<=9;i++)
	{
		cout<<yaoshui[i].size()<<" "<<yaoshui[i]<<endl;
	}
	for(int i=0;i<=9;i++)
	{
	...
	cout<<zhuangbeiwuqi.size()<<" "<<zhuangbeiwuqi<<endl;
	cout<<zhuangbeia<<endl;
	cout<<zhuangbeifangju.size()<<" "<<zhuangbeifangju<<endl;
...
int main() {
	srand((int)time(0));
	char ch;
	if(MessageBox(NULL,"是否读取存档？","艾诺迪亚5:巨岩魔王的觉醒 V0.6",MB_YESNO)==6)
	{
		Read();
		Is_save=true; 
		ch='1';
		system("mode con cols=120 lines=150");
	}
	else
	{
		system("mode con cols=120 lines=150");
	...
	Sleep(100);
	HWND hwnd=GetForegroundWindow();
	MoveWindow(hwnd,100,100,980,650,TRUE);
	start:
	if(!Is_save)
	{
		system("cls");
	...
	if(ch=='1') {
		...
		Slowsay("and who sitting in front of the computer playing this game\n");
		Sleep(3000);
		system("cls");
		Sleep(1500);
	...
	if(ch=='2') {
		...
		cout<<endl;
		cout<<endl;
		if(MessageBox(NULL,"是否前往作者博客查看游戏规则？","艾诺迪亚5:巨岩魔王的觉醒 V0.6",MB_YESNO)==6)
		{
			Slowsay("下面即将跳转至作者的洛谷博客，此操作不含任何危险行为，但可能会被360（划掉）等杀毒软件认为是危险程序，请放心。\n\n");
			system("explorer https://www.luogu.com.cn/blog/wwwbokecom/post-rule");
		}
		Sleep(1000);
		goto start;
	}
